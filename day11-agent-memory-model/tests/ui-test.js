...
(async () => {
  ...
  check("a task is a record of its own, not a field on a chat",
    ...
    storedTasks()[0].status === "open",
    JSON.stringify(storedTasks()[0] && { t: storedTasks()[0].title, s: storedTasks()[0].status }));
  ...
  const stars = qa12("#memory-body .mem-star");
  const unstarred = stars.find((s) => !s.classList.contains("on"));
  check("every item has a star saying whether it always rides", !!unstarred);
  click12(unstarred);
  ...
  // ---- closing the task: a lifetime, not just a scope ----
  const closeBtn = Array.from(q12("#memory-body .task-row").querySelectorAll("button"))
    .find((b) => b.textContent.includes("Close"));
  check("an open task can be closed from the panel", !!closeBtn);
  click12(closeBtn);
  await waitFor(() => storedTasks()[0].status === "closed");
  check("closing keeps every item it learned",
    storedTasks()[0].status === "closed" && storedTasks()[0].items.length === 2,
    JSON.stringify({ s: storedTasks()[0].status, n: storedTasks()[0].items.length }));
  ...
  await waitFor(() => chatBodies().length === 1);
  await waitFor(() => !q12("#chat-inner .bubble.pending"));
  // The last entry only: the one before it is the turn made while the task
  // was still open, and it quite correctly carries the block this checks for.
  const closedRequest = qa12("#debug-panel .debug-entry").slice(-1).map((e) => e.textContent).join("\n");
  check("a closed task stops being sent",
  ...
  check("and knows the task is closed",
