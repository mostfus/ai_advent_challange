"""Day 16: what is actually on the other end of an MCP server.
...
**What this file deliberately does not do**, because the day is "list the
tools" and every one of these belongs to the day that calls one:

  * `tools/call`. Nothing here invokes anything. The catalogue is the product.
  * A held-open connection. See `list_tools` - the session is opened and
    dropped inside one call, which is enough for a catalogue and is not
    enough the moment the server wants to talk back.
  * OAuth. Every server in `KNOWN_SERVERS` is open, and that is not an
...
class Tool:
    ...
    def to_function(self, server_id: str) -> dict:
        ...
        Nothing in this app sends one yet. It is computed and shown anyway,
        because it is the whole argument of the day in one object: a catalogue
        fetched over a protocol nobody at DeepSeek has heard of drops into
        `tools[]` with a rename and no translation.
        """
        return {
            "type": "function",
            "function": {
            ...
            with httpx.Client(timeout=self.timeout, follow_redirects=True) as client:
                result = self.call(client, "initialize", {
                    ...
                    "params": {
                        ...
                        # Empty, and honestly so. This declares what the
                        # *client* can be asked to do - sampling, roots,
                        # elicitation - and this client can be asked to do
                        # none of them. Claiming otherwise would invite a
                ...
                # The negotiated version, not the requested one: two of the
                # five known servers answer with an older one, and what goes
                # in the header from here on is their answer.
                self.protocol = str(result.get("protocolVersion") or PROTOCOL_VERSION)
                info = result.get("serverInfo") or {}
                capabilities = result.get("capabilities") or {}

                if "tools" not in capabilities:
                    # Worth its own message rather than an empty list: a
                    # server with prompts and resources and no tools is
                    # working correctly and has nothing this day wants.
                    raise MCPError("this server does not offer tools")

                self.post(client, "notifications/initialized", {
                    # No `id`. That is what makes it a notification rather
                    # than a request, and why there is no reply to read: a
                    # well-behaved server answers 202 with an empty body.
                    "jsonrpc": "2.0",
