...
#
# One server is seeded, and switched **on**, which breaks the pattern the
...
# of a rule would be doing the one thing day 14 is against. A connected MCP
# server changes nothing: nothing is called, no tool reaches a request, and
# the entire effect of it being on is that opening the panel shows a real
# catalogue from a real server instead of an empty box with a form in it.
mcp_servers = McpStore()
mcp_servers.seed(
    [
        McpServer(
            id=mcp_client.KNOWN_SERVERS[0]["id"],
            label=mcp_client.KNOWN_SERVERS[0]["label"],
            url=mcp_client.KNOWN_SERVERS[0]["url"],
        )
    ]
)
...
# --------------------------------------------------------------------------
# Day 16: MCP - connect to a server, and ask what it can do
# --------------------------------------------------------------------------
