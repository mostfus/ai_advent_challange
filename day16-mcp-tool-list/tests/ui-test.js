...
(async () => {
  ...
  check("one server ships connected, so the panel has something real in it",
    seededMcp.servers.length === 1 && seededMcp.servers[0].id === "deepwiki"
      && seededMcp.servers[0].enabled === true,
    JSON.stringify(seededMcp.servers.map((s) => s.id)));
  check("and it ships with no catalogue - connecting is not contacting",
    Object.keys(seededMcp.servers[0].catalogue).length === 0);
