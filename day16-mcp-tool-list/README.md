...
Day 16 is the first crack in that. An MCP server is a thing on the internet that publishes **what it can do**, in a format anything can read, and the day stops one step short of doing any of it: connect, ask what is there, draw the list. Nothing is called. That is not a shortcut — it is the half that is worth understanding on its own, because the catalogue is the part that makes tool use *discoverable* rather than hard-coded, and the calling half is the same three lines with a different method name.
...
Nothing in this app sends one yet. The schema is on screen under every tool anyway, because that is the shortest available proof that the catalogue drops into a request with a rename and no adaptation.
...
### Five servers that answer without credentials

Offered in the add form, and served from `mcp_client.KNOWN_SERVERS` so a URL cannot rot in the page while the module that knows about it is corrected. All five answered this client on the day it was written:

| Server | URL | Tools | Why it is here |
|---|---|---|---|
| **DeepWiki** | `mcp.deepwiki.com/mcp` | 3 | Questions about any public GitHub repo's wiki. The one that ships connected. |
...
One is seeded, switched **on**, which breaks the pattern the personality and invariant seeds set — both of those land off because they change every answer. A connected MCP server changes nothing: nothing is called, nothing reaches a request, and the entire effect of it being on is that the panel opens onto a real catalogue instead of an empty form.

### What is deliberately not here

`tools/call`, for the reason at the top. `resources/list` and `prompts/list` — three of the five servers advertise both, and it is the same three lines with a different method name. OAuth. And a held-open connection, which is the one of the four that is a design decision rather than a scope line.
