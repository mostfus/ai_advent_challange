...
class Conversation:
    ...
    # Day 9. None until this chat has overflowed its context window at least
    # once - which is also why it is optional on disk: every file written
    # before day 9 simply has no summary, and reads back as a chat that has
    # never needed one.
    ...
    def usages(self) -> list[dict]:
        """Every turn's reported cost, in order - day 8.
        ...
        """
        return [m.usage for m in self.messages if m.usage]
...
class ConversationStore:
    ...
    def save_summary(self, conversation_id: str, summary: Summary) -> Conversation | None:
        """Write down what the older part of a chat has been compressed into.
    ...
    def clear_summary(self, conversation_id: str) -> Conversation | None:
