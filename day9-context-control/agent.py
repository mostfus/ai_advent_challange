...
# Whether the messages that fall outside that window are summarised instead of
# dropped. Off by default: compression costs an extra request of its own, and a
# feature that spends money should be switched on deliberately.
DEFAULT_CONTEXT_COMPRESSION = False
...
class Agent:
    ...
    def build_messages(self, user_message: str) -> list[dict]:
        """`[system] + [summary] + the recent past + the new question`.
        ...
        The summary, when there is one, sits between the two: after the role,
        because it is context and not identity, and before the window, because
        it describes what came *before* those messages and reading it after
        them would put the conversation in the wrong order.
        """
        messages: list[dict] = []
