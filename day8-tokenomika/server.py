...
def build_agent(settings: Settings, conversation: Conversation | None) -> Agent:
    """One short-lived agent, configured and handed the transcript so far.

    Day 7's three lines, minus the third.
    """
    return Agent(
        deepseek_client,
        AgentConfig(
        ...
        history=conversation.history() if conversation else None,
    )
...
def with_usage(conversation: Conversation) -> dict:
    ...
    to them.
    """
    return {**conversation.to_dict(), "usage": usage_report(conversation)}
...
def clear_conversation(conversation_id: str) -> dict:
    ...
    if conversation is None:
        ...
        return {"cleared": True, "messages": [], "usage": usage_report(None)}
    return {
        "cleared": True,
...
def post_chat(req: ChatRequest) -> dict:
    ...
    agent = build_agent(settings, saved)
