...
(async () => {
  ...
  const usageBar = q("#chat-inner .token-bar");
  const usageDetail = q("#chat-inner .token-detail");
  ...
  check("every compare column has its own usage strip",
    panes.every((p) => !!p.querySelector(".token-bar")),
    `${panes.filter((p) => !!p.querySelector(".token-bar")).length} of ${panes.length}`);
  ...
  const demoBar = demoPane.querySelector(".token-bar");
  demoBar.dispatchEvent(new dom4.window.MouseEvent("click", { bubbles: true }));
  const demoDetail = demoPane.querySelector(".token-detail");
  ...
  check("its usage strip came along too", !!q5("#chat-inner .token-bar"));
