# LLM Agent that shows what it spent

A small **agent** that takes a question, sends it to an LLM over plain HTTP, shows the whole exchange in a web GUI, keeps as many conversations as you like and remembers all of them across restarts — and, since day 8, **reports what all of that cost, in the only unit anyone is billed in.**

Day 7 gave the agent a memory by replaying the transcript into every request. That is also the day it quietly started spending more per message than the message is worth: a chat-completions API charges per token, the request carries the whole conversation, and the conversation only grows. Nothing on screen said so. Day 8 is the readout that does.
...
That second number is day 8's actual finding, and it is what day 7's forty-message cap is defending against.
...
server.py             transport, the introduction between a saved transcript
        │              and an agent, and the usage summary on top of both
        ├──────────────────────────────┐
agent.py              the agent:       │  store.py       conversations as JSON
        │  ★ .usage      role, params, │    ★ usage      files, atomic writes,
        │                request, reply│                 one lock
tokens.py         ★ NEW    reads `usage` out of a response, and adds usage up
        │                  (imports nothing from the app)
llm_client.py         HTTP: POST /chat/completions
   DeepSeek
```

`tokens.py` is the bottom of the stack on purpose: it reads and sums dicts, and knows nothing about agents, stores, HTTP or which provider is on the other end. `agent.py` imports it and still imports no storage layer; `store.py` keeps a `usage` dict without doing arithmetic on it — the summing belongs to neither, so it happens in `server.py`.

The day-7 wiring is unchanged — three lines in `server.py`:
...
…with the fourth argument being all of day 8 that touches it.
...
reply.usage      # {"prompt_tokens": …, "completion_tokens": …, "reasoning_tokens": …}
```

Read out of the response and normalised, never computed. The agent assembles the request; the API reports what it came to. Those are two jobs belonging to two different machines, and day 8 does not blur them.
...
The message box is a real `<textarea>`: paste or type text of any length, including line breaks. Press **Enter** to send, **Shift+Enter** for a line break. Directly above it, **Usage** unfolds into what the last turn and the whole chat have cost.
