...
class Agent:
    ...
    def build_messages(self, user_message: str) -> list[dict]:
        """`[system] + [what stands in for the rest] + the window + the question`.
        ...
        The carried-over block, when there is one, sits between the two: after
        the role, because it is context and not identity, and before the
        window, because it accounts for what came *before* those messages and
        reading it after them would put the conversation in the wrong order.

        At most one of the two ever appears. The strategies that produce them
        are exclusive (`strategies.py`), and they have to be: a summary and a
        fact block are two descriptions of the same missing messages, and a
        request carrying both would be asking the model to decide which of
        them had gone stale. The `elif` is where that guarantee is actually
