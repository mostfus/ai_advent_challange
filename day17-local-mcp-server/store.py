...
#: Day 14, and a fifth directory for the same reason there was a fourth. An
#: invariant has a profile's lifetime and a profile's author, so the temptation
#: is to file it next to one - and the whole of day 14 is that those two are
#: not the same kind of thing. `data/personality/` holds what somebody wants;
#: this holds what the work is committed to, which is the difference between a
#: preference and a rule, and it should be visible in a directory listing.
INVARIANTS_DIR_ENV = "CHAT_INVARIANTS_DIR"
DEFAULT_INVARIANTS_DIR = Path(__file__).parent / "data" / "invariants"

#: Day 16, and a sixth. Every directory above holds something somebody in this
#: app wrote down; this one holds a **cache of what somebody else's server
#: said**, which is a different kind of file and should not be mixed in with
#: the five that are authoritative. Delete `data/mcp/` and nothing is lost
#: that a refresh does not fetch again - which is true of none of the others.
MCP_DIR_ENV = "CHAT_MCP_DIR"
DEFAULT_MCP_DIR = Path(__file__).parent / "data" / "mcp"
...
#: How many invariants are kept, and the argument is not `MAX_PROFILES`' one.
#: A profile list is capped because a menu of thirty is unreadable. A rule list
#: is capped because forty rules are not a stricter agent - they are a set that
#: contradicts itself somewhere, with nobody able to say where. Twelve is about
#: as many as one person can hold in mind well enough to notice when two of
#: them disagree. `invariants.MAX_RULES` says the same number for the half of
#: the app that is allowed to have an opinion about why.
MAX_RULES = 12

#: Day 16. Low for the reason every other cap here is low - the panel is a
#: popover, not a page - and lower than it could be because refreshing them
#: all is a serial walk over other people's servers, and eight slow ones is
#: already a long press of one button.
MAX_MCP_SERVERS = 8
...
class Message:
    ...
    `tools` is day 17: the tools this answer ran before it was written, with
    what they returned. Stored on the message for `usage`'s reason - it was
    true once, of this turn, and cannot be recomputed from the text. It is
    also what lets a reloaded page draw the restaurant cards under the answer
    they belong to instead of a paragraph that mentions restaurants.
    """
...
@dataclass
class Rule:
    """One invariant: a sentence the work is committed to, and why.

    A record with no behaviour in it, exactly as `Profile` is. What the kinds
    *are*, how a rule is rendered into a prompt and what counts as breaking
    one all live in `invariants.py`, which this file does not import - so
    `kind` is a plain string here and `markers` a plain list of them, whatever
    the server cleaned and handed down, written back out unchanged.

    `because` is stored beside the rule rather than folded into its text
    because the two are read by different people at different times. The text
    is what an answer is held against; the `because` is what a refusal quotes
    when somebody asks why not, and a rule whose reason nobody wrote down is
    one that gets argued with every time it bites.
    """
    ...
    def from_dict(cls, data: dict) -> "Rule | None":
        """One rule off disk, or None if it is not one.

        A rule with no id is dropped for `Profile.from_dict`'s reason - every
        way of addressing one goes through the id. A rule with no *text* is
        dropped too, which is where this differs: a half-filled profile is a
        legitimate state somebody is in the middle of, while a rule with
        nothing in it would be numbered, sent and enforced against, and there
        is nothing there to enforce.
        """
        if not isinstance(data, dict):
            return None
        ...
        markers = [str(m).strip() for m in (raw if isinstance(raw, list) else ()) if str(m).strip()]
        created = str(data.get("created_at") or "") or now_iso()
        return cls(
            id=rule_id,
...
@dataclass
class Invariants:
    """Every rule, and whether the set as a whole is in force.

    One record rather than a list, for the reason `Personality` keeps `active`
    beside its profiles: the switch is meaningless without the thing it
    switches. It is two switches deep on purpose - the set has one and each
    rule has one - and the two answer different questions. A rule switched off
    is one that has stopped being true; the set switched off is the day
    itself turned off, which is how it gets demonstrated: ask with the rules
    off, switch them on, ask again.
    """

    rules: list = field(default_factory=list)
    enabled: bool = True

    def find(self, rule_id: str | None):
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "Invariants":
        if not isinstance(data, dict):
            return cls()
        raw = data.get("rules")
        rules = []
        ...
        return cls(
            ...
            # Absent reads as on. A file written before this field existed is
            # a file whose rules somebody meant, and defaulting it to off
            # would silently stop enforcing them.
            enabled=bool(data.get("enabled", True)),
        )


def invariants_dir_from_env() -> Path:
    """Where the rules are kept: `CHAT_INVARIANTS_DIR`, or `data/invariants`."""
    configured = os.getenv(INVARIANTS_DIR_ENV)
    return Path(configured).expanduser() if configured else DEFAULT_INVARIANTS_DIR


class InvariantStore:
    """The rules, on disk: one file, read on every request.

    Structurally `PersonalityStore` again - one file, a lock, write-to-temp
    and `os.replace` - and read exactly as often, because this one also
    contributes to every request rather than to the ones it matches. There is
    no per-conversation copy for the same reason there is none of a profile:
    a rule the work is committed to does not stop applying because you opened
    a new chat, and a chat that remembered which rules were in force when it
    started would let you escape one by starting another.
    """

    FILENAME = "rules.json"

    def __init__(self, directory: Path | str | None = None) -> None:
        self.directory = Path(directory) if directory else invariants_dir_from_env()
        self.directory.mkdir(parents=True, exist_ok=True)
        self.path = self.directory / self.FILENAME
        self._lock = threading.RLock()

    def load(self) -> Invariants:
        """Everything on file. An unreadable file reads as no rules at all.

        Which is the one read in this class that is arguably the wrong way
        round: everywhere else today an invariant fails closed. It cannot here
        - a file that will not parse has no rules *to* enforce, and the
        alternative to returning an empty set is refusing to answer anything
        at all until somebody fixes a file by hand.
        """
        with self._lock:
            if not self.path.exists():
                return Invariants()
            try:
                ...
                return Invariants()
        return Invariants.from_dict(data)

    def save(self, record: Invariants) -> Invariants:
        payload = json.dumps(record.to_dict(), ensure_ascii=False, indent=2)
        with self._lock:
            tmp = self.path.with_suffix(".json.tmp")
            tmp.write_text(payload, encoding="utf-8")
    ...
    def seed(self, rules: list) -> Invariants:
        """Write these rules, but only if nothing has ever been written.

        The condition is the *file*, not an empty list - `PersonalityStore.
        seed`'s rule, and it matters more here. Deleting every shipped rule is
        exactly what somebody does on their way to writing their own, and an
        app that put two back on the next restart would be adding rules to
        their project that nobody agreed to.
        """
        with self._lock:
            if self.path.exists():
                return self.load()
            # Off, and see `invariants.SEED_RULES` for why: these describe the
            # app they shipped with, and enforcing them against whatever the
            # user is actually building would be the wrong default in the one
            # direction this day cannot afford to be wrong in.
            return self.save(Invariants(rules=list(rules), enabled=False))

    def upsert(self, rule: Rule) -> Invariants:
        """File a rule, replacing any with the same id.

        The replacement keeps its position, as `PersonalityStore.upsert` does,
        and here that is not only about a menu holding still: position within
        a kind is what the prompt numbers, so a rule that jumped to the end
        when it was edited would renumber the rules either side of it and make
        every earlier refusal in the transcript cite the wrong line.
        """
        with self._lock:
            record = self.load()
            for index, existing in enumerate(record.rules):
                if existing.id == rule.id:
                    ...
                    record.rules[index] = rule
                    return self.save(record)
            if len(record.rules) >= MAX_RULES:
                raise StoreError(f"No room for another invariant (limit {MAX_RULES})")
            record.rules.append(rule)
            return self.save(record)

    def remove(self, rule_id: str) -> Invariants:
        """Delete a rule. There is nothing else to fix up: unlike a profile,
        no pointer anywhere names one."""
        with self._lock:
            record = self.load()
            record.rules = [r for r in record.rules if r.id != rule_id]
            return self.save(record)

    def set_enabled(self, enabled: bool) -> Invariants:
    ...
    def set_rule_enabled(self, rule_id: str, enabled: bool) -> Invariants:
        """Switch one rule on or off, leaving it on file either way."""
        with self._lock:
            record = self.load()
            rule = record.find(rule_id)
            if rule is None:
                return record
...
# --------------------------------------------------------------------------
# Day 16: the MCP servers, and the last thing each of them said
# --------------------------------------------------------------------------


@dataclass
class McpServer:
    """A server somebody connected, plus the catalogue it last handed back.

    Two halves with different owners, and keeping them in one record is the
    point rather than a shortcut. `label`, `url` and `enabled` are **ours** -
    typed in, and true until changed. `catalogue` is **theirs** - whatever
    `mcp_client.Listing.to_dict` produced on the last visit, written down
    unread and handed back unread.

    That is the same contract `Rule.kind` has with `invariants.py`: this file
    does not know what a tool is, what a JSON Schema means or which protocol
    version was negotiated, and it should not learn, because the moment it
    does there are two places that have an opinion about the wire format and
    one of them is a file on disk.

    **Why the catalogue is stored at all**, when a refresh would fetch it
    again: so that the panel has something to draw before the network does,
    and - the part that matters more - so that "when was this last true" is a
    question with an answer. A catalogue that only ever existed in memory
    would make every restart look like every server had just gone down.
    """
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "url": self.url,
            "enabled": bool(self.enabled),
            "catalogue": dict(self.catalogue or {}),
            "created_at": self.created_at,
            "updated_at": self.updated_at,
        }

    @classmethod
    def from_dict(cls, data: dict) -> "McpServer | None":
        """One server off disk, or None if it is not one.

        Dropped for no id or no url, on `Rule.from_dict`'s reasoning: the id is
        how every route addresses one, and a server with no address is a row
        whose refresh button cannot do anything.
        """
        if not isinstance(data, dict):
            return None
        ...
        return cls(
            id=server_id,
            label=str(data.get("label") or "").strip() or server_id,
            url=url,
            enabled=bool(data.get("enabled", True)),
...
def mcp_dir_from_env() -> Path:
    """Where the connected servers are kept: `CHAT_MCP_DIR`, or `data/mcp`."""
    configured = os.getenv(MCP_DIR_ENV)
    return Path(configured).expanduser() if configured else DEFAULT_MCP_DIR


class McpStore:
    """The connected servers, on disk: one file, a lock, atomic replace.

    Structurally `InvariantStore` once more, and read far less often - nothing
    here contributes to a request. It is read when the panel is drawn and
    written when a listing comes back, which is the shape of a cache rather
    than of a setting, and is why `record_listing` exists as its own method:
    the one write that is not somebody's decision.
    """

    FILENAME = "servers.json"

    def __init__(self, directory: Path | str | None = None) -> None:
        self.directory = Path(directory) if directory else mcp_dir_from_env()
        self.directory.mkdir(parents=True, exist_ok=True)
        self.path = self.directory / self.FILENAME
        self._lock = threading.RLock()

    def load(self) -> list:
        """Every server on file. An unreadable file reads as none connected."""
        with self._lock:
            if not self.path.exists():
                return []
            try:
        ...
        servers = []
        for entry in raw if isinstance(raw, list) else ():
            server = McpServer.from_dict(entry)
            if server is not None:
                servers.append(server)
        return servers[:MAX_MCP_SERVERS]

    def save(self, servers: list) -> list:
        payload = json.dumps(
            {"servers": [s.to_dict() for s in servers], "updated_at": now_iso()},
            ensure_ascii=False,
            indent=2,
        )
        with self._lock:
    ...
    def find(self, server_id: str) -> "McpServer | None":
        for server in self.load():
            if server.id == server_id:
                return server
        return None

    def seed(self, servers: list) -> list:
        """Write these, but only if nothing has ever been written.

        `InvariantStore.seed`'s condition and `InvariantStore.seed`'s reason:
        disconnecting the server that shipped with the app is a thing somebody
        does on purpose, and putting it back on the next restart would be the
        app reconnecting to a third party they had just disconnected from.

        Unlike the seeded rules, this one is seeded **on**. A rule that is on
        changes every answer; a connected server that nobody has called does
        nothing at all until the panel is opened, and arriving to an empty
        panel would hide the one thing the day has to show.
        """
        with self._lock:
            if self.path.exists():
                return self.load()
            return self.save(list(servers))

    def upsert(self, server: McpServer) -> list:
        """File a server, replacing any with the same id and keeping its place."""
        with self._lock:
            servers = self.load()
            for index, existing in enumerate(servers):
                if existing.id == server.id:
                    server.created_at = existing.created_at
                    # A re-added server keeps what it last said, so the panel
                    # does not blank out between the write and the refresh.
                    if not server.catalogue:
                        server.catalogue = existing.catalogue
                    servers[index] = server
                    return self.save(servers)
            if len(servers) >= MAX_MCP_SERVERS:
                raise StoreError(f"No room for another MCP server (limit {MAX_MCP_SERVERS})")
            servers.append(server)
            return self.save(servers)

    def remove(self, server_id: str) -> list:
        """Disconnect one. There is nothing to clean up elsewhere: no session
        is held, so there is no connection to close - which is the one part of
        this day that a pool of live clients would have made complicated."""
        with self._lock:
            servers = [s for s in self.load() if s.id != server_id]
            return self.save(servers)

    def set_enabled(self, server_id: str, enabled: bool) -> list:
        """Switch one off without forgetting it, or its catalogue."""
        with self._lock:
            servers = self.load()
    ...
    def record_listing(self, server_id: str, catalogue: dict) -> list:
        """Write down what a server just said - success or failure alike.

        A failed listing is stored rather than discarded, which is the whole
        reason this is not simply `upsert`. Dropping it would leave the last
        good catalogue on screen with a stale timestamp under it, and the
        panel would quietly claim a server is up for as long as it stays down.
        """
        with self._lock:
            servers = self.load()
            for server in servers:
                if server.id == server_id:
                    server.catalogue = dict(catalogue or {})
                    server.updated_at = now_iso()
                    return self.save(servers)
            return servers
