...
import invariants as invariants_module
import mcp_client
...
from invariants import CheckRun, InvariantWatcher
...
# Day 14, and a fifth store, read on every request exactly as the fourth is.
# The parallel with `personalities` is close enough to be worth stating where
# it stops: both hold something the user declared, both are global rather than
# per-chat, both are written only when somebody edits them. What differs is
# what the app does with them afterwards - a profile is applied and that is
# the end of it, while these are applied *and then the answer is read back
# against them*, which is the whole of day 14 and the only reason it needed a
# file of its own.
#
# The two seeds land switched off, exactly as the personality ones do. They
# are there so the panel teaches the shape of a rule before anybody has
# written one - and not in force, because they describe *this* app and would
# otherwise hold somebody's unrelated project to the architecture of the thing
# they just cloned. "Before" is half the demonstration here too.
invariants = InvariantStore()
invariants.seed(
    [
        Rule(
            id=invariants_module.unique_id(seed["text"], []),
            **invariants_module.clean_rule(seed),
        )
        for seed in invariants_module.SEED_RULES
    ]
)


# Day 16, and a sixth store - the first one in this app that holds something
# nobody here wrote. The five above are all the user's own text; this is a
# list of other people's servers and a cache of what they last said when
# asked what they can do.
#
# Two servers are seeded, and switched **on**, which breaks the pattern the
# personality and invariant seeds set. Both of those land off because they
# change every answer, and an app that silently applied somebody else's idea
# of a rule would be doing the one thing day 14 is against.
#
# On day 16 a connected server changed nothing at all - nothing was called and
# no tool reached a request, so the entire effect of it being on was that the
# panel opened onto a real catalogue. Day 17 makes it a real choice: the tools
# of a server that is on ride on every request this app sends, which costs
# tokens whether or not they are used. It is still seeded on, because the day
# it demonstrates cannot be seen with everything switched off - and the switch
# is one click away in the panel that shows exactly what it is offering.
mcp_servers = McpStore()
mcp_servers.seed(
    [
        McpServer(id=known["id"], label=known["label"], url=known["url"])
        # The first two: this folder's own Google Maps server (day 17) and
        # DeepWiki (day 16). One of ours and one of somebody else's, because
        # the point of a catalogue is that the agent cannot tell which is
        # which - it is handed function definitions either way.
        for known in mcp_client.KNOWN_SERVERS[:2]
    ]
)
...
@dataclass
class Toolbox:
    """The tools this turn may use, and how to reach the one it picks.

    Two halves of the same fact. `functions` is what goes into the request -
    `mcp_client.Tool.to_function` for every tool on every connected server,
    already in chat-completions shape. `routes` is how a name comes back: the
    model answers with `google-maps__find_restaurants` and something has to
    know that this means *that* tool on *that* URL.

    Built fresh per turn, off the stored catalogues, so switching a server off
    in the panel takes effect on the next message and not on the next restart.
    Nothing is fetched here - a request that had to visit five servers before
    it could ask the model anything would be a page that hangs for seconds
    because somebody's documentation server is slow.
    """

    functions: list = field(default_factory=list)
    routes: dict = field(default_factory=dict)

    def run(self, function_name: str, arguments: dict):
        """Call the tool behind a qualified name. The agent's `tool_runner`.

        A name with no route is the one case this cannot pass to a server, and
        it still answers rather than raising: the model invented a tool, and
        being told so is something it can act on.
        """
        route = self.routes.get(function_name)
        if route is None:
            return mcp_client.ToolResult(
                ok=False, is_error=True,
                text=f"Error: there is no tool called {function_name!r}.",
...
def build_toolbox() -> Toolbox:
    ...
    The same set `mcp_report` counts, which is deliberate: the number on the
    button is the number of tools the next message will carry, and a panel
    that counted one thing while the agent was offered another would make the
    whole catalogue untrustworthy.
    """
    box = Toolbox()
    for server in mcp_servers.load():
        ...
        for raw in server.tools():
            ...
            if name in box.routes:
                # Two tools that qualify to the same 64 characters. The first
                # one keeps the name; the second is dropped rather than
                # silently shadowing it.
                continue
            box.routes[name] = (server.id, server.url, tool.name)
            box.functions.append(function)
    return box
...
class Settings(BaseModel):
    ...
    # Day 14. Whether anything reads the answer back against the invariants.
    #
    # The exact parallel of the switch above, and it governs the *checking*
    # rather than the rules. Off, the rules still ride on every request and
    # the model is still told it may not break one - what stops is the second
    # request that reads the answer, and with it the retry. So turning it off
    # does not relax an invariant, it demotes one: from a rule that is
    # enforced back to a rule that is merely stated, which is precisely what
    # day 12 already had and what this day exists to improve on.
...
def build_agent(
    ...
    invariant_record: Invariants | None = None,
    invariant_correction: str | None = None,
    ...
    return Agent(
        ...
        # Day 14, arriving as its own parameter for exactly the reason the
        # profile above does: `ContextPlan` answers "what of the past goes
        # into this request", and a rule the work is committed to is not part
        # of the past. The correction is appended the way day 13's is - the
        # block says what the rules are, and the paragraph after it says which
        # one the last attempt broke.
        invariants=(
            invariants_module.render(invariant_record)
            + ("\n\n" + invariant_correction if invariant_correction else "")
            if invariant_record is not None
            else None
        ),
        # Day 17, and the first argument here that is not a block of prose.
        # Everything above tells the agent what it knows; this tells it what
        # it can *do* - and, like all of them, it arrives already in the shape
        # the agent needs, so the agent never learns that MCP was involved.
        tools=toolbox.functions if toolbox else None,
        tool_runner=toolbox.run if toolbox else None,
    )
...
def rule_report(record: Invariants, rule: Rule) -> dict:
    """One invariant, as the panel draws it.

    `number` is served rather than derived in the browser, and it is the one
    field here that would be easy to get wrong locally. The numbering is
    across the whole live set in `KINDS` order, it skips rules that are
    switched off, and it is what the prompt, the watcher's verdict and every
    refusal in the transcript all cite. A page that counted its own rows would
    eventually label a rule `3` while the model was being told it was `2`.
    """
    return {
        "id": rule.id,
        "kind": invariants_module.clean_kind(rule.kind),
        "text": rule.text,
        "because": rule.because,
        "markers": list(rule.markers or []),
        "enabled": bool(rule.enabled),
        "number": invariants_module.number_of(record, rule.id),
        "summary": invariants_module.summary(rule),
        "updated_at": rule.updated_at,
    }


def invariants_report(record: Invariants | None = None) -> dict:
    """Every rule, whether the set is on, and the block that goes with requests.

    The whole of it comes back from every write below, for `personality_report`'s
    reason and one more of its own: switching a single rule off renumbers all
    the rules after it, so there is no such thing as a partial update to this
    panel. Returning a delta would mean the page recomputing the numbering,
    which is the one thing it must not do.
    """
    record = record if record is not None else invariants.load()
    return {
        "rules": [rule_report(record, rule) for rule in record.rules],
        "enabled": bool(record.enabled),
        "in_force": len(invariants_module.active(record)),
        "limit": MAX_RULES,
        "room": max(0, MAX_RULES - len(record.rules)),
        "block": invariants_module.render(record),
    }
...
def invariants_debug(run: CheckRun | None) -> dict | None:
    """One checker request, for the debug log - the same shape as the others.

    `hits` is served beside `violations` because the pair is the day in one
    line. Three markers found and none of them a violation is the checker
    working perfectly, and it is indistinguishable from the checker doing
    nothing unless both halves are on screen.
    """
    if run is None:
        return None
    return {
        ...
        "hits": [h.to_dict() for h in run.hits],
        "retried": run.retried,
...
def as_json(
    ...
    return {
        ...
        # Day 14: which rules this request was answered under, sent with every
        # reply for `personality`'s reason - a rule switched off in another tab
        # should be visible on the screen showing the answer it stopped
        # constraining.
        "invariants": invariants_report(invariant_record),
        # What was broken this turn, if anything, and enough to draw the
        # receipt under the answer with: which rule, whose side it came from,
        # and whether the answer on screen is the second one.
        "violations": (
            {
                "found": [v.to_dict() for v in check.violations],
                "retried": check.retried,
        ...
        # Day 17. What this turn actually ran, in order - drawn under the
        # answer as the cards and the call receipts. It is the same list that
        # was just stored on the message, so a reload shows the same thing.
        "tools": reply.tool_calls,
        # And what was on offer: how many tools the request carried, from how
        # many servers. Sent even when nothing was called, because "it had
        # them and chose not to" and "it had none" are different answers to
        # the only question anybody asks of a tool-using agent.
        "toolbox": {
            "offered": len(reply.request.get("tools") or []) if reply.request else 0,
        },
        "branching": branch_report(conversation),
        ...
        "debug": {
            ...
            "usage": reply.usage,
            # Day 17: the rounds of the tool loop *before* the one that
            # produced the answer. The request above is the last one; these
            # are the ones that asked for a tool instead, and they were billed
            # too - which is the part of tool use that is easiest to hide.
            "rounds": reply.rounds,
            ...
            # Day 14: the other after-the-answer request, and the only one in
            # this app that can replace the answer outright. Day 13's changes
            # a field on disk; this one changes what you are reading.
            "invariants": invariants_debug(check),
        },
    }
...
def get_config() -> dict:
    ...
    return {
        ...
        # Day 14, and the fifth time. The four kinds, their labels and their
        # placeholders come from `invariants.KIND_INFO` for the reason all
        # four before it did: a page carrying its own copy of the kinds would
        # eventually offer one the server drops on the way in.
        "invariant_kinds": [
            {"id": name, **invariants_module.KIND_INFO[name]}
            for name in invariants_module.KINDS
        ],
        "invariant_limit": MAX_RULES,
        "invariant_text_limit": invariants_module.MAX_TEXT_CHARS,
        "invariant_because_limit": invariants_module.MAX_BECAUSE_CHARS,
        # Day 16, and the sixth time - with the least riding on it and the
        # most obvious payoff. The suggestions in the add form are the five
        # servers `mcp_client` was actually tested against, served from there
        # so that a URL cannot rot in the page while the module that knows
        # about it is corrected.
        "mcp_known": list(mcp_client.KNOWN_SERVERS),
        "mcp_limit": MAX_MCP_SERVERS,
        "mcp_protocol": mcp_client.PROTOCOL_VERSION,
    }
...
# --------------------------------------------------------------------------
# Invariants: the day-14 endpoints
# --------------------------------------------------------------------------


class RuleRequest(BaseModel):
    """One invariant, on the way in.

    Everything is defaulted so a minimal `curl` body still parses, and nothing
    here is trusted: `invariants.clean_rule` decides what a rule is, and both
    the create and the update handler go through it. `markers` is typed loose
    because the form sends a comma-separated line and the API takes a list -
    `clean_markers` accepts either, so the page does not have to grow a tag
    widget to talk to its own server.
    """
...
class ToggleRequest(BaseModel):
    """On or off - for one rule, or for the whole set."""

    enabled: bool = True


class ScanRequest(BaseModel):
    """Some text to run the markers over, and nothing else."""
...
def get_invariants() -> dict:
    """Every rule, and the block that will ride on the next request."""
    return invariants_report()


@app.post("/api/invariants")
def create_rule(req: RuleRequest) -> dict:
    """Write a new invariant. Created in force, unlike a personality profile.

    That asymmetry is deliberate and it is about what the two things are. A
    profile created and not activated is a draft you are trying out; an
    invariant is a statement that the work is *already* committed to
    something, and one filed switched-off would be a rule nobody is keeping
    that also is not being enforced - a note, written in the panel for rules.
    """
    cleaned = invariants_module.clean_rule(req.model_dump())
    if not cleaned:
        raise HTTPException(400, "An invariant needs something in it")
    record = invariants.load()
    rule = Rule(
        id=invariants_module.unique_id(cleaned["text"], [r.id for r in record.rules]),
        **cleaned,
    )
    try:
        return invariants_report(invariants.upsert(rule))
    except StoreError as exc:
        raise HTTPException(400, str(exc))


@app.put("/api/invariants/{rule_id}")
def update_rule(rule_id: str, req: RuleRequest) -> dict:
    """Edit an invariant, keeping its id.

    The id survives a rewrite for `update_profile`'s reason and a sharper one:
    an id here is derived from the rule's own text, so re-deriving it on every
    edit would mean fixing a typo in a rule silently replaced it with a
    different rule - and `upsert` would then file the new one at the end,
    renumbering everything after it.
    """
    record = invariants.load()
    existing = record.find(rule_id)
    if existing is None:
        raise HTTPException(404, f"No such invariant: {rule_id}")
    cleaned = invariants_module.clean_rule(req.model_dump())
    if not cleaned:
        raise HTTPException(400, "An invariant needs something in it")
    rule = Rule(id=existing.id, created_at=existing.created_at, **cleaned)
    try:
        return invariants_report(invariants.upsert(rule))
    except StoreError as exc:
        raise HTTPException(400, str(exc))
...
@app.post("/api/invariants/{rule_id}/toggle")
def toggle_rule(rule_id: str, req: ToggleRequest) -> dict:
    """Switch one rule off without deleting it.

    The difference between this and `delete_rule` is the difference between a
    rule that has stopped being true and one that was never right. Both leave
    the answer unconstrained; only one of them is still on the screen a
    fortnight later when somebody asks why the agent started proposing
    databases again.
    """
    if invariants.load().find(rule_id) is None:
        raise HTTPException(404, f"No such invariant: {rule_id}")
    return invariants_report(invariants.set_rule_enabled(rule_id, req.enabled))


@app.post("/api/invariants/enabled")
def set_invariants_enabled(req: ToggleRequest) -> dict:
    """Switch the whole set on or off.

    Off is a state rather than an empty list - day 12's rule about `active`,
    and here it is what makes the day demonstrable at all: ask a question with
    the rules off, switch them on, ask it again. Nothing is deleted either
    way, so switching back on restores exactly the set that was there.
    """
    return invariants_report(invariants.set_enabled(req.enabled))


@app.delete("/api/invariants/{rule_id}")
def delete_rule(rule_id: str) -> dict:
    """Delete a rule. Nothing points at one, so nothing has to be fixed up."""
    return invariants_report(invariants.remove(rule_id))


@app.post("/api/invariants/scan")
def scan_text(req: ScanRequest) -> dict:
    """What the deterministic half alone sees in some text.

    The smallest endpoint in the file and the one that does the most to keep
    this day honest. `invariants.scan` is the part that costs nothing and
    needs no model, so it is the part that can be *tested* - the UI suite runs
    against a dummy key where every model call is a 401, and without this
    there would be no way to assert that any of the checking works.

    It returns hits, not violations, and the difference is the module's whole
    argument: "we are not using Postgres here" hits the marker and breaks no
    rule. What this says is *where to look*, which is exactly what it says to
    the watcher.
    """
    record = invariants.load()
    hits = invariants_module.scan(record, req.text)
    return {
        "hits": [
            {**hit.to_dict(), "number": invariants_module.number_of(record, hit.rule_id)}
            for hit in hits
        ],
        "checked": len(invariants_module.active(record)),
    }


# --------------------------------------------------------------------------
# Days 16 and 17: MCP - connect to a server, ask what it can do, call it
# --------------------------------------------------------------------------


class McpRequest(BaseModel):
    """A server somebody is connecting: a name for it and where it lives."""

    label: str = Field("", max_length=60)
    url: str = Field("", max_length=400)


def mcp_slug(label: str, url: str, taken: list) -> str:
    """A short id for a server, unique among the ones already connected.

    It is not cosmetic. This string is the namespace half of every tool name
    the catalogue reports - `deepwiki__ask_question` - so it is built out of
    the same character class `mcp_client.qualify` will allow and kept short,
    because whatever it costs comes out of the 64 characters a function name
    gets.
    """
    base = re.sub(r"[^a-z0-9]+", "-", (label or url).lower()).strip("-")[:24]
    base = base or "server"
    ...
    for suffix in range(2, 100):
        ...
        if candidate not in taken:
            return candidate
    raise HTTPException(status_code=400, detail="Could not name that server")


def clean_mcp_url(url: str) -> str:
    """The one check on the way in, and it is about this app rather than MCP.

    Pressing refresh makes *the server* fetch a URL a browser typed, which is
    a request originating inside whatever network this is running on. The
    scheme check below is not a defence against that - a private address over
    https passes it - and it is not pretending to be one. It is here so the
    failure for a typo is a sentence in the panel instead of an httpx
    traceback, and the real answer is that this is a local single-user app and
    the URL box is as trusted as the terminal it was launched from. An app
    with more than one user would need an allow-list here.
    """
    url = (url or "").strip()
    if not url:
        raise HTTPException(status_code=400, detail="A server needs a URL")
    if not url.startswith(("http://", "https://")):
        raise HTTPException(status_code=400, detail="The URL must start with http:// or https://")
    return url


def mcp_report(servers: list | None = None) -> dict:
    """Every connected server, what it last said, and the totals.

    The whole set on every reply, as the invariants endpoints do and for a
    version of the same reason: connecting a server renames nothing, but the
    counts in the header and the chip on the button are sums over all of them,
    and a page that patched one row would have to recompute those itself.
    """
    servers = mcp_servers.load() if servers is None else servers
    rows = [s.to_dict() for s in servers]
    live = [s for s in servers if s.enabled and s.catalogue.get("ok")]
    return {
        "servers": rows,
        # What an agent would actually be offered: tools from servers that
        # are switched on and answered. A tool on a server switched off is
        # still drawn in the panel and is deliberately not counted here.
        "tools": sum(len(s.tools()) for s in live),
        "connected": len(live),
        "limit": MAX_MCP_SERVERS,
        "room": max(0, MAX_MCP_SERVERS - len(servers)),
    }


def fetch_catalogue(server: McpServer) -> dict:
    """Go and ask one server what it can do, and write down the answer.

    The only place in this file that opens an MCP session, and it does not
    keep it - `MCPClient.list_tools` connects, reads and hangs up. A failure
    comes back as a stored listing rather than an exception, so a server that
...
@app.get("/api/mcp")
def get_mcp() -> dict:
    """What is connected, off the cache - no server is contacted.

    Drawing the panel must not fetch. Five servers at a second or two each is
    five seconds of a popover that opens empty, and the catalogue it would
    fetch is one a refresh can get on purpose. What this returns is the last
    thing each server said and when, which is what a cache is for.
    """
    return mcp_report()


@app.post("/api/mcp")
def add_mcp(req: McpRequest) -> dict:
    """Connect a server - and go and ask it, once, immediately.

    The fetch is here rather than behind a second click because a row that
    appears saying "never checked" answers none of the questions somebody had
    when they pasted a URL in. The one that matters is whether it works at
    all, and this is the moment to find out.
    """
    url = clean_mcp_url(req.url)
    existing = mcp_servers.load()
    if any(s.url == url for s in existing):
        raise HTTPException(status_code=400, detail="That server is already connected")
    label = (req.label or "").strip() or url
    server = McpServer(
        id=mcp_slug(req.label, url, [s.id for s in existing]),
        label=label[:60],
        url=url,
    )
    try:
...
def refresh_mcp(server_id: str) -> dict:
    """Ask one server again. The only button here that touches the network."""
    server = mcp_servers.find(server_id)
    if server is None:
...
@app.post("/api/mcp/{server_id}/toggle")
def toggle_mcp(server_id: str, req: ToggleRequest) -> dict:
    """Switch a server off without forgetting it.

    Distinct from deleting it in the one way that matters here: the catalogue
    survives. A server switched off keeps its tools on screen, greyed, and
    stops contributing them to the count - which is the difference between
    "not offering these" and "never heard of these".
    """
    return mcp_report(mcp_servers.set_enabled(server_id, req.enabled))


@app.delete("/api/mcp/{server_id}")
def delete_mcp(server_id: str) -> dict:
    """Disconnect. Nothing is held open, so nothing has to be closed."""
    return mcp_report(mcp_servers.remove(server_id))
...
def post_chat(req: ChatRequest) -> dict:
    ...
    # Day 14, read here for exactly the reason the profile above is: so the
    # reply reports the same rules the request was built from. A rule switched
    # off between the two reads would otherwise put an answer on screen under
    # a set of invariants it was never held to.
    invariant_record = invariants.load()
    # Day 17, read here for the same reason as the two above: the tools this
    # answer was offered are reported back with it, and a server switched off
    # mid-turn should not make the screen claim a tool the request never
    # carried. Nothing is fetched - this is the stored catalogue.
    toolbox = build_toolbox()
    agent = build_agent(settings, plan, profile_record.current(),
                        invariant_record=invariant_record, toolbox=toolbox)
    ...
    if advice is not None and advice.rejected:
        second = build_agent(
            ...
            invariant_record=invariant_record,
            # The retry gets the same tools. It is a new answer to the same
            # question, and one that could not look anything up would be a
            # worse answer held to a stricter standard - it also means a
            # rejected turn can pay for the tool calls twice, which is the
            # honest price of asking again rather than editing.
            toolbox=toolbox,
        ).ask(message)
    ...
    # Day 14, and it happens here for day 13's reason twice over: before the
    # turn is stored, because the answer may not survive it - and *after* the
    # stage retry above, because what has to be checked is the answer that
    # would actually be shown. Judging the first draft of an answer that was
    # already thrown away would be checking a thing nobody will ever read.
    #
    # The honest cost, since it is easy to hide: a turn that trips both
    # subsystems buys three answers and two side requests. It is not collapsed
    # into one retry, because they refuse for unrelated reasons - and the
    # stage retry's answer is a new answer that nothing has checked yet.
    check = run_invariant_check(settings, invariant_record, message, reply)
    if check is not None and check.rejected:
        second = build_agent(
            settings, plan, profile_record.current(),
            invariant_record=invariant_record,
            invariant_correction=invariants_module.correction(
                invariant_record, check.violations
            ),
            toolbox=toolbox,
        ).ask(message)
        # A retry that itself fails leaves the first answer standing, exactly
        # as above. It is the answer that broke a rule, which is bad - but
        # replacing it with a transport error would punish the user twice for
    ...
    conversation = store.append_turn(
        ...
        usage=reply.usage,
        # Day 17: what this answer ran to be able to say what it says, stored
        # with it. A reload redraws the cards; nothing has to be called again.
        tools=reply.tool_calls,
        branch=branch_id,
    )
    ...
    return as_json(reply, conversation, plan, proposals,
                   ...
                   invariant_record=invariant_record, check=check)
...
def run_invariant_check(
    ...
    """Read the answer back against the rules. Never raises.

    Runs *before* the turn is stored, for `run_advisor`'s reason and more
    bluntly: the caller may throw the answer away on what this returns, and an
    answer that broke a rule must never reach the disk, because the transcript
    is replayed into later requests. One stored violation becomes an example
    the model follows for the rest of the conversation.

    Skipped in three cases, and each is a rule rather than a saving:

    * the setting is off - the rules still went up with the request, nothing
      reads the answer back. See `Settings.invariant_check`;
    * there are no rules in force, or the set is switched off - there is
      nothing to check against, and paying for a model to confirm that is
      the clearest waste in the file;
    * **the turn failed** - an error message is not evidence about anything.
      A 401 does not break an invariant, and a checker handed one would be
      reading a transport failure for opinions about architecture.
    """
    if not settings.invariant_check or reply.error is not None:
        return None
    if not invariants_module.numbered(record):
        return None
    return InvariantWatcher(deepseek_client, settings.model).check(
        record, message, reply.answer
    )
