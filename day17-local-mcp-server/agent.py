...
Day 17 changes the one thing eleven days left alone: `ask` is a **loop**. The
agent is handed a list of tool definitions and one callable that runs them
(`tools=`, `tool_runner=`), and when the model answers with `tool_calls`
instead of words, each is run, its result is appended to `messages` as a
`tool` message, and the same question is asked again. Nothing here knows what
MCP is, which server a tool lives on or how a session is opened - it is handed
two objects, exactly as it is handed a client and a history, and the loop
would run identically over functions defined in this file.
...
#: Day 17. How many times one question may go round the tool loop before the
#: agent stops asking and answers with what it has. Each round is a full
#: chat-completions request, so this is a budget, not a safety net: two
#: restaurant searches and a distance is three, and a model looping on a tool
#: that keeps failing is the case it is actually here for.
MAX_TOOL_ROUNDS = 5
...
# Day 13, and the block that is about *now* rather than about what is known.
# Everything above it answers "what is true"; this one answers "where has this
# got to, and what is expected next" - a stage, a checklist and one sentence
# saying what happens now.
#
# It sits immediately after the working layer because it has exactly that
# scope: both are about one task and neither is about this conversation. The
# order between the two is the order they are read in - what the task has
# established, then where the task has got to - and the state block is the
# narrower of them, because a fact filed in planning outlives the stage that
# filed it.
#
# The label says "not a summary" out loud for a reason. A model handed a block
# of context about a task, with no instruction attached, reliably does the one
# thing this day exists to stop: it restates it. `phases.render` closes with a
# plain "carry on from here, do not start again", and this prefix is where
# that instruction starts.
STATE_PREFIX = (
...
# Day 14, and the only block that is not addressed to the model's judgement
# at all. Every block above is something to take into account; this one is a
# list of things it is not permitted to do, and the difference shows up in
# what happens when it is ignored - an ignored preference produces an answer
# that reads wrong, an ignored invariant produces one that is thrown away
# before anybody sees it.
#
# It goes **last**, after personalisation, and that is the one placement here
# worth arguing about. The block above ends by claiming that where it
# disagrees with anything above *it* wins - which it must not, against these.
# A preference that contradicts a decision the work is already committed to is
# the preference that yields. Later context is weighted more heavily, so the
# block nothing may override is the one that sits closest to the question.
#
# The label names its author for `PERSONALITY_PREFIX`'s reason, and adds the
# one fact that makes the rules bind rather than merely appear: the answer is
# read back. A model told that a rule is enforced behaves differently from one
# told that a rule is important.
INVARIANTS_PREFIX = (
    "--- Invariants, set by the user. These are not preferences ---\n"
)
...
class AgentReply:
    ...
    # Day 17. Every tool this turn ran, in order, as `server.py` recorded
    # them: which server, which tool, what arguments, what came back. Empty
    # for a turn that called nothing, which is most of them.
    tool_calls: list = field(default_factory=list)
    # The earlier rounds of the tool loop - one entry per chat-completions
    # request that ended in "call this tool" rather than in an answer. The
    # last round is `request`/`response` above; these are the ones before it,
    # and they are kept so the debug panel can show a turn that cost three
    # requests as three requests.
...
class Agent:
    ...
    def __init__(
        ...
        # Day 13: the task's stage, its checklist and what is expected next,
        # rendered by `phases.render` exactly as the four above are rendered
        # elsewhere. The agent is handed a block of text and a place to put
        # it, and does not know that a machine decided what could go in it -
        # which is what keeps "there is now a fifth stage" a change to
        # `phases.py` and no change at all to this class.
        ...
        # Day 14: the rules an answer is not allowed to break, rendered by
        # `invariants.render` exactly as the five above are rendered
        # elsewhere. The agent is handed text and a place to put it, and does
        # not know that something reads its answer back against the same
        # rules - which is what keeps "the checker now catches paraphrase" a
        # change to `invariants.py` and no change at all to this class.
        self.invariants = (invariants or "").strip() or None
        # Day 17, and the first thing ever handed to this class that is not
        # text. `tools` is the `tools[]` array of a chat-completions request -
        # already in that shape, because `mcp_client.Tool.to_function` found
        # that MCP's tool schema *is* that shape. `tool_runner` is a callable
        # `(function_name, arguments) -> mcp_client.ToolResult`.
        #
        # Both come from outside for the reason every other block does: the
        # agent's job is to run the loop, not to know that MCP exists, which
        # server a tool came from or how a session is opened. Hand it two
        # functions off a local file and it behaves identically.
    ...
    def build_messages(self, user_message: str) -> list[dict]:
        ...
        **widest scope first**:
            ...
            task state    where this task has got to           (day 13)
            summary|facts true for this conversation
            personality   not about the past at all - who is asking
            invariants    not about the past either - what may not be done
        ...
        Day 13's block is the exception that proves the ordering: it is not a
        statement about what is true at all, it is a statement about where the
        work has got to, and it goes directly after the layer with the same
        scope. What the task has established, then where the task stands.
        ...
        Day 14's block goes after even that, and the two are not rivals for
        the slot - they are the two halves of what the user declared, in the
        order of how hard they bind. A profile says how an answer should
        read and a model drifting there is not a bug; an invariant says what
        an answer may not contain, and drifting there is the whole failure.
        So the block that nothing may override is the one nearest the
        question, and personalisation's claim to beat "anything above" stops
        ...
        if self.state:
            messages.append({"role": "system", "content": STATE_PREFIX + self.state})
        if self.summary:
        ...
        if self.invariants:
            messages.append({"role": "system", "content": INVARIANTS_PREFIX + self.invariants})
    ...
    def build_request(self, user_message: str, messages: list[dict] | None = None) -> dict:
        """The exact JSON body this agent would POST for `user_message`.

        Separate from `ask` so it can be inspected (and tested) without
        spending a token. `messages` is day 17's one addition: the tool loop
        asks the same question several times with more messages each time, and
        passes the grown list in rather than having it rebuilt from a history
        that does not contain tool traffic.
        """
        body: dict = {
            "model": self.config.model,
            "messages": messages if messages is not None else self.build_messages(user_message),
        }

        # Day 17. Offered, never forced: `tool_choice` is left at the API's
        # default of "auto", so a question that needs no tool costs exactly
        # what it cost yesterday plus the tokens these definitions take. That
        # is the honest cost of the day and it is not small - two tools with
        # full JSON Schemas are a few hundred tokens on *every* request, which
        # is why the panel's switch off a server is a real economy.
        if self.tools:
    ...
    def ask(self, user_message: str) -> AgentReply:
        ...
        Day 17 makes this a **loop** rather than a call, and that is the whole
        of the day on this side. The shape is the one every tool-using agent
        has, and it is worth reading as a shape rather than as code:

            ask  ->  the model answers, or asks for a tool
                     |                       |
                     |                       run it, append the result,
                     |                       ask again with it in the messages
                     v
                   the answer

        Two properties of that loop are decisions rather than mechanics:

        **The model is never told what a tool returned except by being asked
        again.** There is no branch here that reads a result and writes the
        answer itself - the result goes back into `messages` as a `tool`
        message and the model is asked the same question with more of the
        world in front of it. That is why five restaurants come back as a
        recommendation and not as a table: nothing here formats anything.

        **A failed tool is an answer, not an error.** `ToolResult.is_error` is
        put in the `tool` message exactly as a success is, because "Google
        found no walking route across the Baltic" is something a model can
        say back to a person, and an exception here would be this app
        deciding, on the model's behalf, that the turn was over.

        Errors from the *LLM* still never propagate: a failed call comes back
        as an `AgentReply` whose `answer` is the explanation, because every
        caller here needs to put something in a chat bubble.
        """
        ...
        for round_index in range(MAX_TOOL_ROUNDS + 1):
            request_body = self.build_request(user_message, messages)
            try:
            ...
            if usage:
                usages.append(usage)
            message = ((result.response.get("choices") or [{}])[0] or {}).get("message") or {}
            requested = message.get("tool_calls") or []

            # Either the model answered, or it has run out of rounds. The last
            # round is the one reported as `request`/`response`, which is what
            # makes the debug panel show the request that produced the words
            # on screen rather than the first of three.
            if not requested or not self.can_run_tools or round_index == MAX_TOOL_ROUNDS:
                answer = self.extract_answer(result.response)
                if requested and self.can_run_tools:
                    # Out of rounds with the model still asking. Said out loud
                    # rather than silently answered, because a turn that hit
                    # the budget is a turn whose answer may be missing the
                    # thing it was about to look up.
                    answer = (answer or "").strip() or (
                        f"(stopped after {MAX_TOOL_ROUNDS} tool rounds - "
                        "the model kept asking for more tools)"
                    )
                ...
                return AgentReply(
                    ...
                    usage=tokens.sum_usage(usages) if usages else None,
                    tool_calls=tool_calls,
                    rounds=rounds,
                )

            # A round that asked for tools. It is kept whole for the panel -
            # this request and this reply are as real as the final pair, and
            # they were billed.
            rounds.append({
                "request": result.request,
                "response": result.response,
            ...
            # The assistant's turn goes back verbatim, including the
            # `tool_calls` it made: the `tool` messages below are replies to
            # ids in here, and an API that cannot match them refuses the next
            # request. `reasoning_content` rides along when the provider sent
            # one - DeepSeek's thinking models expect their own reasoning back
            # inside a tool loop.
            ...
            for call in requested:
                record, content = self.run_tool(call)
        ...
        raise AssertionError("unreachable: the loop returns on its last round")

    @property
    def can_run_tools(self) -> bool:
        return bool(self.tools and self.tool_runner)

    def run_tool(self, call: dict) -> tuple[dict, str]:
        """One entry of `tool_calls`, run - and what to put in the `tool` message.

        Everything that can go wrong before the tool is reached is handled
        here and handled the same way: as a `tool` message saying so. A model
        that sent unparseable arguments can fix them on the next round; an
        exception raised here could only end the turn.
        """
        function = call.get("function") or {}
        name = str(function.get("name") or "")
        raw = function.get("arguments")
        record = {"id": call.get("id") or "", "name": name, "arguments": {}}

        try:
            arguments = json.loads(raw) if isinstance(raw, str) and raw.strip() else (raw or {})
        except ValueError as err:
            record["error"] = f"the model sent arguments that are not JSON ({err})"
            record["ok"] = False
            return record, f"Error: {record['error']}. Send valid JSON arguments."
        if not isinstance(arguments, dict):
            record["error"] = "arguments were not a JSON object"
            record["ok"] = False
            return record, "Error: arguments must be a JSON object."
        record["arguments"] = arguments

        try:
            result = self.tool_runner(name, arguments)
        except Exception as err:  # noqa: BLE001 - a broken runner is a tool failure
            record["ok"] = False
            record["error"] = f"{type(err).__name__}: {err}"
            return record, f"Error: the tool could not be called ({err})"

        record.update(result.to_dict() if hasattr(result, "to_dict") else {"raw": result})
        record["ok"] = bool(getattr(result, "ok", False))
        return record, result.for_model()
